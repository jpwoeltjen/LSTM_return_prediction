...
def stochastic_vol_day_edge_generator_lf_bigger(annual_drift, annual_sd, years, edge_probability, edge_intesity):
    '''
    ...
    above (below) the mean if the volume the day before increased (decreased) by >2 standard deviations. It returns a DataFrame with the time series and the expected profit and sharpe ratio p.a. from the edge (only).
    '''
    length = 252*years+1 
    ...
    sim_data['up_edge'] = ((sim_data['random']<edge_probability)&((sim_data['volume'].shift(2)-sim_data['volume'].shift(3)) > 2*vol_sd)&
            (sim_data['day_of_business_week']==0)).astype(int)
    sim_data['down_edge'] = ((sim_data['random']<edge_probability)&((sim_data['volume'].shift(2)-sim_data['volume'].shift(3)) < -2*vol_sd)&
            (sim_data['day_of_business_week']==0)).astype(int)
...
sim_data, expected_profit_pa, expected_edge_sharpe_ratio= stochastic_vol_day_edge_generator_lf_bigger(0.0, 0.1, 30, 0.9, 3)  
sim_data.to_csv('data/ae5.csv', header = False, index=True, encoding='utf-8')
